...
import math
import random
from src.Service.Perso3.MeteoriteService import meteorite
from src.Service.Perso3.PunchService import punch

class Perso3(pygame.sprite.Sprite):
    def __init__(self,game,num,bot):
        ...
        self.animations[0] = pygame.transform.scale(
            pygame.image.load("src/Service/Perso3/graphismes/Marche/marche6.png"), (150, 200))
        self.animations[1] = []
        self.animations[2] = []
        for i in range(1, 7):
            self.animations[1].append(
                pygame.transform.scale(pygame.image.load(f"src/Service/Perso3/graphismes/Marche/marche{i}.png"),
                                       (150, 200)))
        for i in range(1, 5):
            self.animations[2].append(
                pygame.transform.scale(pygame.image.load(f"src/Service/Perso3/graphismes/Punch/punch{i}.png"),
                                       (150, 200)))
        self.animation_time = 100
        self.action = 0
        self.frame = 0
        self.health = self.max_health
        self.meteoriteDmg = 20
        self.fistDmg = 15
        self.fireball_freeze = 30
        self.punch_freeze = 10
    ...
    def parade_on(self):
        if not self.freeze :
            ...
            self.image = pygame.image.load("src/Service/Perso3/graphismes/Ryu_parade.png")
    ...
    def low_freeze(self):
        if self.freeze:
            self.freeze -= 1
            if self.freeze >= 1:
                self.image = pygame.image.load("src/Service/Perso3/graphismes/Ryu_freezed.png")
                self.image = pygame.transform.scale(self.image, (150, 200))
                if self.direction == 0:
                    self.image = pygame.transform.flip(self.image, True, False)
            else :
    ...
    def attack_spe(self):
        if not self.parade and not self.cooldown and not self.freeze:
            self.projectiles.add(meteorite(self,self.game,self.numb_meteorites))
            self.cooldown = 250
